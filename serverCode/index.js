...
const run = async () => {
  ...
  try {
    ...
    app.put("/task/:id", async (req, res) => {
      ...
      const target = await taskCollection.findOne(filter);

      const updateDoc = {
        $set: {
          isCompleted: task?.isCompleted,
          title: task?.title || target.title,
          description: task?.description || target.description,
          priority: task?.priority || target.priority,
        },
      };
